...
class ReloadPopularMoviesReducerTest {
    ...
    fun `reduce reload popular movies success`() = runTest {
        ...
        val (homeFeatureState, effect) = homeFeatureReducer.reduce(appState, HomeAction.ReloadPopularMovies)

        assertTrue(homeFeatureState.popularMovies.isLoading)

        val movies = listOf(ModelUtil.movieDataModel)
        val dataSuccessMovies = DataState.Success(movies)
        whenever(mockExecutorHolder.movieRemoteSource.nowPopularMovies()).thenReturn(dataSuccessMovies)
        ...
        verify(executor.effectExecutorScope).dispatch(
            HomeAction.PopularMoviesLoaded(
    ...
    @Test
    fun `reduce reload popular movies api error`() = runTest {
        val homeFeatureEffects = HomeFeatureEffects(testDispatcher)
        val homeFeatureReducer = HomeFeatureReducer(homeFeatureEffects)
        val appState = AppState.INITIAL
        val (homeFeatureState, effect) = homeFeatureReducer.reduce(appState, HomeAction.ReloadPopularMovies)
        ...
        whenever(mockExecutorHolder.movieRemoteSource.nowPopularMovies()).thenReturn(dataErrorMovies)
    ...
    @Test
    fun `reduce reload popular movies network error`() = runTest {
    ...
    @Test
    fun `reduce reload popular movies unknown error`() = runTest {
