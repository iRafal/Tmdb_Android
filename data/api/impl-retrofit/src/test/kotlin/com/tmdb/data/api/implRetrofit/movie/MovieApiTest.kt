package com.tmdb.data.api.implRetrofit.movie
...
import com.tmdb.data.api.implRetrofit.di.module.ApiFactoriesModule
import com.tmdb.data.api.implRetrofit.di.module.ApiHttpClientModule
import com.tmdb.data.api.implRetrofit.di.module.ApiJsonModule
import com.tmdb.data.api.implRetrofit.di.module.ApiModule
import com.tmdb.data.api.implRetrofit.util.ModelUtil
...
class MovieApiTest {
    ...
    @[OptIn(ExperimentalSerializationApi::class) BeforeTest]
    fun setup() {
        mockWebServer.start()
    ...
    fun `movie by id api error no body`() = runTest {
        ...
        assertTrue(response.isUnknownError)
        assertTrue(response is ApiResponse.UnknownError)
        assertTrue(response.cause is ApiException.HttpError)
        assertEquals((response.cause as ApiException.HttpError).code, errorCode)
    }
    ...
    fun `now playing movies success`() = runTest {
        ...
        assertTrue(response.isSuccess)
        assertEquals(
            expected = (response as ApiResponse.Success).data,
            actual = DataPage(page = 1, results = listOf(movieModel), totalPages = 1, totalResults = 1)
        )
    }
